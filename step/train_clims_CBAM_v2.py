...
def sub_cam_eval(args, model):
    ...
    model_cam = getattr(importlib.import_module(args.clims_network), 'CAM_CBAM_V2')(n_classes=20)
...
def run(args):
    ...
    model = getattr(importlib.import_module(args.clims_network), 'CLIMS_CBAM_V2')(n_classes=20)
    ...
    layers = [] 
    for i in range(3):
        layers.append(model.stage1[4][i].ca)
        layers.append(model.stage1[4][i].sa)
    for i in range(4):
        layers.append(model.stage2[0][i].ca)
        layers.append(model.stage2[0][i].sa)
    for i in range(6):
        layers.append(model.stage3[0][i].ca)
        layers.append(model.stage3[0][i].sa)
    for i in range(3):
        layers.append(model.stage4[0][i].ca)
        layers.append(model.stage4[0][i].sa)
    ignore = layers.copy()
    ignore.append(model.classifier)
    
    ignore_layers = nn.ModuleList(ignore)
    special_layers = nn.ModuleList(layers)
    
    special_layers_params = list(map(id,special_layers.parameters()))
    ignore_layers_params = list(map(id,ignore_layers.parameters()))
    
    base_params = filter(lambda p: id(p) not in ignore_layers_params ,model.parameters())
    
    param_groups = model.trainable_parameters()
    optimizer = torchutils.PolyOptimizer([
        {'params': base_params, 'lr': args.clims_learning_rate, 'weight_decay': args.cam_weight_decay},
        {'params': model.classifier.parameters(), 'lr': 10 * args.clims_learning_rate, 'weight_decay': args.cam_weight_decay},
         {'params': special_layers.parameters(), 'lr': 1000 * args.clims_learning_rate, 'weight_decay': args.cam_weight_decay},
    ...
    optimizer = torchutils.PolyOptimizer([
        ...
        {'params': param_groups[1], 'lr': 10 * args.clims_learning_rate, 'weight_decay': args.cam_weight_decay},
         {'params': param_groups[2], 'lr': 100 * args.clims_learning_rate, 'weight_decay': args.cam_weight_decay},
    ], lr=args.clims_learning_rate, weight_decay=args.cam_weight_decay, max_step=max_step)
    '''
    ...
    for ep in range(args.clims_num_epoches):
        ...
        for step, pack in enumerate(train_data_loader):
            ...
            L_CPS =  L_REG#torch.mean(torch.abs(cam1 - cam2))
            #L_CPS = torch.mean(torch.square(cam1 - cam2))
            
            #print(cam1.shape)
            
            if(args.clims_all):
                loss = hyper[0] * L_OTM + hyper[1] * L_BTM + hyper[2] * L_CBS + hyper[3] * L_REG #+ args.z*L_CPS
            else:
                loss = hyper[0] * L_OTM + hyper[1] * L_BTM  + hyper[3] * L_REG + args.z*L_CPS
