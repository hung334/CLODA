...
def run(args):
    ...
    for ep in range(args.clims_num_epoches):
        ...
        for step, pack in enumerate(train_data_loader):
            ...
            x ,cam1 = model(img)
            x2,cam2 = cam_wood.wood(img)
            
            L_CLS_W = F.multilabel_soft_margin_loss(x2, label)
            ...
            if(args.clims_all):
                loss = hyper[0] * L_OTM + hyper[1] * L_BTM + hyper[2] * L_CBS + hyper[3] * L_REG + args.s*L_CPS + args.z*L_CLS_W #+ args.z*L_CLS
            else:
            ...
            avg_meter.add({'loss1': loss.item(), 'L_OTM': L_OTM.item(), 'L_BTM': L_BTM.item(), 'L_CBS': L_CBS.item(),
                           'L_REG': L_REG.item(),
                           'L_CPS': L_CPS.item(),'L_CLS': L_CLS.item(),'L_CLS_W': L_CLS_W.item(),
                          })
            ...
            if (optimizer.global_step - 1) % 200 == 0:
                ...
                print('step:%5d/%5d' % (optimizer.global_step - 1, max_step),
                      ...
                      'L_CLS_W:%.4f' % (avg_meter.pop('L_CLS_W')),
