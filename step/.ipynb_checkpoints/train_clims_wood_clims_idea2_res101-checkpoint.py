...
def sub_cam_eval(args, model):
    ...
    model_cam = getattr(importlib.import_module(args.clims_network), 'CAM_CBAM')(n_classes=20)
...
def run(args):
    ...
    #print('args.clims_network',args.clims_network)
    #model = getattr(importlib.import_module(args.clims_network), 'CLIMS')(n_classes=20)
    model = getattr(importlib.import_module(args.clims_network), 'CLIMS_CBAM')(n_classes=20)
    cam_wood = getattr(importlib.import_module(args.clims_network), 'CLIMS_wood')(n_classes=20)
    res50_wood = './wood_pth/res101_cam_ood_5337.pth'
    #'./wood_pth/res50_cam_ood_V3_52468.pth'
    #'./wood_pth/res50_cam_ood_V3_52231.pth'
    if(args.org_cam):
        res50_cam ='wood_pth/res101_cam_5118.pth'#'./wood_pth/res50_cam_ood_V3_52231.pth'
    else:
        res50_cam =res50_wood 
    
    #res50_cam = './result_resnet101/resnet101_ep10/res101_cam_5118.pth'
    ...
    for ep in range(args.clims_num_epoches):
        ...
        for step, pack in enumerate(train_data_loader):
            ...
            #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            _, _, h, w = x.size()
            cls_x =  F.avg_pool2d(x, kernel_size=(h, w), padding=0)#x.view(-1,20)
            #print(cls_x.shape)
            
            L_CLS = F.multilabel_soft_margin_loss(cls_x.view(-1,20), label)
            ...
            L_CPS =  torch.mean(torch.abs(cam1 - cam2))
            ...
            avg_meter.add({'loss1': loss.item(), 'L_OTM': L_OTM.item(), 'L_BTM': L_BTM.item(), 'L_CBS': L_CBS.item(),
                           'L_REG': L_REG.item(),
                           'L_CLS': L_CLS.item(),'L_CPS': L_CPS.item(),
                          })
            ...
            if (optimizer.global_step - 1) % 200 == 0:
                ...
                print('step:%5d/%5d' % (optimizer.global_step - 1, max_step),
                      ...
                      'L_CBS:%.6f' % (avg_meter.pop('L_CBS')),
                      'L_REG:%.4f' % (avg_meter.pop('L_REG')),
                      'L_CPS:%.6f' % (avg_meter.pop('L_CPS')),
