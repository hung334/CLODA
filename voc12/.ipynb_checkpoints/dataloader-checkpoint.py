...
class VOC12ImageDataset(Dataset):
    ...
    def __init__(self, img_name_list_path, voc12_root,
                 ...
                 crop_size=None, crop_method=None, to_torch=True,saliency_root='../Sal-VOC/Sal/',sal_normal=TorchvisionNormalize(mean=(0, 0, 0), std=(1, 1, 1))):
        ...
        self.crop_size = crop_size #crop_size=512, 
        ...
        self.sal_normal = sal_normal
        
        self.blip_normal = TorchvisionNormalize(mean=(0.48145466, 0.4578275, 0.40821073), std=(0.26862954, 0.26130258, 0.27577711))
    ...
    def __getitem__(self, idx):
        ...
        #sal_img = cv2.imread(os.path.join(self.saliency_root,name_str+'.png'),0)#np.asarray(imageio.imread(os.path.join(self.saliency_root,name_str+'.png')))#HW
        #
                                                                        
        #img =  {'image':input_img , 'saliency':sal_img}                                            
        if self.resize_long:#resize_long=(320, 640)
            
            #img = imutils.random_resize_long(img, self.resize_long[0], self.resize_long[1])
            #sal_img = imutils.random_resize_long(sal_img, self.resize_long[0], self.resize_long[1])
            
                        
            target_long = random.randint(self.resize_long[0], self.resize_long[1])
            img = imutils.random_resize_long(img,target_long)
            #sal_img = imutils.random_resize_long(sal_img,target_long)

        if self.rescale:
            img = imutils.random_scale(img, scale_range=self.rescale, order=3)
            #sal_img = imutils.random_scale(sal_img, scale_range=self.rescale, order=3)
            
        if self.img_normal:#TorchvisionNormalize()
            img = self.img_normal(img)
            blip = self.blip_normal(img)
            ##sal_img = self.sal_normal(sal_img,)
            
            #imgarr = np.asarray(sal_img)
            #sal_img = np.empty_like(imgarr, np.float32)
            #sal_img[...] = (imgarr[...] / 255.)
            

        if self.hor_flip:#hor_flip=True,
            #img = imutils.random_lr_flip(img)
            #sal_img  = imutils.random_lr_flip(sal_img)
            
            a = random.getrandbits(1)

            img = imutils.random_lr_flip(img,a)
            blip = imutils.random_lr_flip(blip,a)
            #sal_img  = imutils.random_lr_flip(sal_img,a)
            
        if self.crop_size:
            if self.crop_method == "random":#crop_method="random"
                img ,box= imutils.random_crop(img, self.crop_size, 0)
                blip = imutils.random_crop_sal(blip, self.crop_size, 0,box)
                #sal_img = imutils.random_crop_sal(sal_img, self.crop_size, 0,box)
            else:
        ...
        if self.to_torch:
            ...
            blip = imutils.HWC_to_CHW(blip)
            #ic(sal_img.shape)
            #sal_img = np.expand_dims(sal_img, 2)#HWC
            #sal_img = imutils.HWC_to_CHW(sal_img)
            

        return {'name': name_str, 'img': img}#,'blip':blip}#,'sal':sal_img}
...
class VOC12SegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.hor_flip:
            ...
            #img = imutils.random_lr_flip(img,a)
            img, label = imutils.random_lr_flip((img, label),a)

        if self.crop_method == "random":
            (img, label),box = imutils.random_crop((img, label), self.crop_size, (0, 255))
        else:
            img = imutils.top_left_crop(img, self.crop_size, 0)
